...
topicCommand      = "iot-2/type/iotqs-sensor/id/080027649fd8/evt/ApplicationControl/fmt/json"
sampleClientID    = "d:quickstart:iotqs-sensor:F61209F5CED2"
commandClientID   = "a:quickstart:iotqs-sensor:080027649fd8"
...
HDLC_PKT_MARKER           = "7E"
START_STREAMING_CMD_HDR   = "7E21"
START_STREAMING_CMD_MSG   = "000201017E"
STOP_STREAMING_CMD_HDR    = "7E21"
STOP_STREAMING_CMD_MSG    = "000202017E"
STOP_STREAMING_RSP_HDR    = "A1"
STOP_STREAMING_RSP_MSG    = "0000"
...
def ConnectIoTAdapter():
    ...
    if(commandHandle == None):
        # Client settings for Receiving Commands.
        commandHandle               = mqtt.Client()
        commandHandle.on_connect    = ConnectCommandHandler
        commandHandle.on_message    = CommandMessageHandler
        commandHandle.on_disconnect = DisconnectHandler
        commandHandle._client_id    = commandClientID
        commandHandle.user_data_set(comHandle)

        # Open Cloud connections.
        sampleHandle.connect(cloudURLEntry.get(), int(cloudPortEntry.get()), keepAliveDuration)
        commandHandle.connect(cloudServerURL, cloudServerPort, keepAliveDuration)

        # Start listening for Events from Cloud.
        sampleHandle.loop_start()
        commandHandle.loop_start()
        while(sampleConnected == False or commandConnected == False):
            time.sleep(1)
...
def ExitDashboard(pauseBeforeExit=False):
    ...
    if comHandle != None:
        comHandle.close()
    if commandHandle != None:
        commandHandle.unsubscribe(topicCommand)
        commandHandle.loop_stop()
        commandHandle.disconnect()
    if sampleHandle != None:
...
# The callback for when the client receives a CONNACK response from the server.
def ConnectCommandHandler(client, userData, flags, rC):
    global commandConnected
    global topicCommand
    if (rC != mqtt.MQTT_ERR_SUCCESS):
        LogToTextBox("Error: Connection with Cloud Service for Commands Failed! [Error Code = %d]" % rC)
        return
    else:
        LogToTextBox("Connected to Cloud Service for receiving Commands...")

    # Subscribing in on_connect() means that if we lose the connection and reconnect then subscriptions will be renewed.
    (retCode, _) = client.subscribe(topicCommand)
    if (retCode != mqtt.MQTT_ERR_SUCCESS):
        LogToTextBox("Error: Subscription to Topic for receiving Commands Failed! [Error Code = %d]" % retCode)
    else:
        LogToTextBox("Subscribed to Topic for receiving Commands from Cloud...\n")
...
def CommandMessageHandler(client, userData, msg):
    global commandCount
    global bridgeActive

    msgStr = re.sub('0x', '', re.sub('[^A-Za-z0-9]+', '', msg.payload)).lower()
    # We send HDLC encoded commands directly to the Board.
    if (msgStr.startswith(HDLC_PKT_MARKER.lower())):
        if(msgStr[:4].upper() == START_STREAMING_CMD_HDR and msgStr[6:].upper() == START_STREAMING_CMD_MSG):
            StartReadingFormComPort()
        elif(msgStr[:4].upper() == STOP_STREAMING_CMD_HDR and msgStr[6:].upper() == STOP_STREAMING_CMD_MSG):
            StopReadingFromComPort()
        else:
            userData.write(msgStr.decode("hex"))
            LogToTextBox("Received HDLC Command #%d [%s] from Cloud." % (commandCount, msg.payload))
        commandCount += 1
    # Process recognised commands.
    elif (exitCommand.lower() in msg.payload.lower()):
        LogToTextBox("\nReceived Command #%d %s from Cloud." % (commandCount, msg.payload))
        commandCount += 1
        ExitDashboard(pauseBeforeExit=True)
    else:
        LogToTextBox("Error: Unrecognised Command [%s] received from Cloud, ignoring..." % msg.payload)
