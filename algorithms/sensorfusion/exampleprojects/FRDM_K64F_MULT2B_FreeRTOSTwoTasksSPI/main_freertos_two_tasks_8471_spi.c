...
/*! \file main_freertos_two_tasks_8471_spi.c
...
#include "fsl_dspi.h"           // KSDK header file for DSPI interfaces
#include "fxls8471q.h"          // register and bit-field definitions
#include "mag3110.h"            // register and bit-field definitions
...
registerDeviceInfo_t spiBusInfo = {
    .deviceInstance     = SPI_S_DEVICE_INDEX,
    .functionParam      = NULL,
    .idleFunction       = NULL
};
...
int main(void)
{
    ...
    ARM_DRIVER_SPI* SPIdrv = &SPI_S_DRIVER;       // defined in the frdm_<xxxx>.h file
    ...
    SPIdrv->Initialize(SPI_S_SIGNAL_EVENT);    // Initialize the SPI driver
    SPIdrv->PowerControl(ARM_POWER_FULL);      // Set the SPI Power mode.
    SPIdrv->Control(ARM_SPI_MODE_MASTER | ARM_SPI_CPOL1_CPHA0, SPI_S_BAUDRATE);
    ...
    //sfg.installSensor(&sfg, &sensors[0], MMA8652_I2C_ADDR,    1, (void*) I2Cdrv, MMA8652_Init,   MMA8652_Read);
    sfg.installSensor(&sfg, &sensors[0], SPI_ADDR,            1, (void*) SPIdrv, &spiBusInfo, FXLS8471Q_Init, FXLS8471Q_Read);
    ...
    sfg.installSensor(&sfg, &sensors[3], MAG3110_I2C_ADDRESS, 1, (void*) I2Cdrv, &i2cBusInfo, MAG3110_Init,   MAG3110_Read);
