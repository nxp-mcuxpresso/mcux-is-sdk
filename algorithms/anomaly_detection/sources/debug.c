...
void ApplyPerturbation(Globals *gbls)
{
    ...
    quaternionPacketType = gbls->pControlSubsystem->QuaternionPacketType;

    Quaternion  ftmpq;  // scratch quaternion

    // calculate the test perturbation
    switch (gbls->iPerturbation)
    {
...
#if F_3DOF_G_BASIC
    case (Q3):
        CurrentQ = gbls->SV_3DOF_G_BASIC.fLPq;
        qAeqAxB(&(gbls->SV_3DOF_G_BASIC.fLPq), &ftmpq);
        break;
#endif
#if F_3DOF_B_BASIC
    case (Q3M):
        CurrentQ = gbls->SV_3DOF_B_BASIC.fLPq;
        qAeqAxB(&(gbls->SV_3DOF_B_BASIC.fLPq), &ftmpq);
        break;
#endif
#if F_3DOF_Y_BASIC
    case (Q3G):
        CurrentQ = gbls->SV_3DOF_Y_BASIC.fq;
        qAeqAxB(&(gbls->SV_3DOF_Y_BASIC.fq), &ftmpq);
        break;
#endif
#if F_6DOF_GB_BASIC
    case (Q6MA):
        CurrentQ = gbls->SV_6DOF_GB_BASIC.fLPq;
        qAeqAxB(&(gbls->SV_6DOF_GB_BASIC.fLPq), &ftmpq);
        break;
#endif
#if F_6DOF_GY_KALMAN
    case (Q6AG):
        CurrentQ = gbls->SV_6DOF_GY_KALMAN.fqPl;
        qAeqAxB(&(gbls->SV_6DOF_GY_KALMAN.fqPl), &ftmpq);
        break;
#endif
#if F_9DOF_GBY_KALMAN
    case (Q9):
        CurrentQ = gbls->SV_9DOF_GBY_KALMAN.fqPl;
        qAeqAxB(&(gbls->SV_9DOF_GBY_KALMAN.fqPl), &ftmpq);
        break;
#endif
    default:
        CurrentQ.q0 = 1.0;
    ...
    case 0:  // no test in progress, check to see if we should start one
        if (gbls->iPerturbation>0) {
            // Start Test
            iTestProgress = 1;
            gbls->iPerturbation = 0;
            iTestDelay = 0;
            //iTestAngle = 0;
