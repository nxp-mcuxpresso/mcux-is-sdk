/*
* Copyright (c) 2015 - 2016, Freescale Semiconductor, Inc.
* All rights reserved.
*
* SPDX-License-Identifier: BSD-3-Clause
*/

/*! \file driver_FXLS8952.c
    \brief Provides init() and read() functions for the FXLS8952 3-axis accelerometer
*/

#include "board.h"                      // generated by Kinetis Expert.  Long term - merge sensor_board.h into this file
#include "sensor_fusion.h"              // Sensor fusion structures and types
#include "sensor_io_i2c.h"              // Required for registerreadlist_t / registerwritelist_t declarations
#include "sensor_drv.h"                 // Contains sensor state and error definitions
#include "FXLS8952.h"                   // describes the FXLS8952 register definition and its bit mask
#include "drivers.h"                    // Device specific drivers supplied by NXP (can be replaced with user drivers)
#define FXLS8952_COUNTSPERG 512
#define FXLS8952_ACCEL_FIFO_SIZE 32

#if F_USING_ACCEL

// Command definition to read the WHO_AM_I value.
const registerreadlist_t    FXLS8952_WHO_AM_I_READ[] =
{
    { .readFrom = FXLS8952_WHO_AM_I, .numBytes = 1 }, __END_READ_DATA__
};

// Command definition to read the number of entries in the accel FIFO.
const registerreadlist_t    FXLS8952_BUF_STATUS_READ[] =
{
    { .readFrom = FXLS8952_BUF_STATUS, .numBytes = 1 }, __END_READ_DATA__
};

// Command definition to read the number of entries in the accel FIFO.
registerreadlist_t          FXLS8952_DATA_READ[] =
{
    { .readFrom = FXLS8952_BUF_X_LSB, .numBytes = 6 }, __END_READ_DATA__
};

// Each entry in a RegisterWriteList is composed of: register address, value to write, bit-mask to apply to write (0 enables)
const registerwritelist_t   FXLS8952_Initialization[] =
{
    // write 0000 0000 = 0x00 to SENS_CONFIG1 to place FXLS8952 into standby
    // [7-1]: Set when taking the part out of standby
    // [0]: ACTIVE=0 for standby
    { FXLS8952_SENS_CONFIG1, 0x00, 0x00 },

    // write 0010 0000 = 0x20 to BUF_CONFIG1 to enable FIFO in continuous (circular) mode
    // [7]: BUF_TYPE=0 for FIFO (not FILO) operation
    // [6-5]: BUF_MODE[1-0]=01 for FIFO continuous mode
    // [4]: BUF_GATE=0 to flush buffer on transitions
    // [3]: TRIG_SDCD_AND=0
    // [2]: TRIG_SDCD_OR=0
    // [1] Reserved=0
    // [0]: TRIG_ORIENT=0
    { FXLS8952_BUF_CONFIG1, 0x20, 0x00 },

    // write 0101 0000 = 0x50 to SENS_CONFIG2 register to set High Performance Mode
    // [7-6]: WAKE_PM=01 for High Performance Mode
    // [5-4]: SLEEP_PM=01 for High Performance Mode
    // [3]: LE_BE=0 for Little Endian
    // [2]: BLOCK=0 for normal latching
    // [1]: AINC_TEMP=0 to not include temperature in auto increment
    // [0]: F_READ=0 for normal 12 bit reads
    { FXLS8952_SENS_CONFIG2, 0x50, 0x00 },

     // write XXXX 0000 = to SENS_CONFIG3 register to set ODR
    // [7-4]: WAKE_ODR=1100 for 0.781Hz ODR and 4096x decimation from 3200Hz giving 0xC0
    // [7-4]: WAKE_ODR=1011 for 1.56Hz ODR and 12048x decimation from 3200Hz giving 0xB0
    // [7-4]: WAKE_ODR=1010 for 3.125Hz ODR and 1024x decimation from 3200Hz giving 0xA0
    // [7-4]: WAKE_ODR=1001 for 6.25Hz ODR and 512x decimation from 3200Hz giving 0x90
    // [7-4]: WAKE_ODR=1000 for 12.5Hz ODR and 256x decimation from 3200Hz giving 0x80
    // [7-4]: WAKE_ODR=0111 for 25Hz ODR and 128x decimation from 3200Hz giving 0x70
    // [7-4]: WAKE_ODR=0110 for 50Hz ODR and 64x decimation from 3200Hz giving 0x60
    // [7-4]: WAKE_ODR=0101 for 100Hz ODR and 32x decimation from 3200Hz giving 0x50
    // [7-4]: WAKE_ODR=0100 for 200Hz ODR and 16x decimation from 3200Hz giving 0x40
    // [7-4]: WAKE_ODR=0011 for 400Hz ODR and 8x decimation from 3200Hz giving 0x30
    // [7-4]: WAKE_ODR=0010 for 800Hz ODR and 4x decimation from 3200Hz giving 0x20
    // [7-4]: WAKE_ODR=0001 for 1600Hz ODR and 2x decimation from 3200Hz giving 0x10
    // [7-4]: WAKE_ODR=0000 for 3200Hz ODR and 1x decimation from 3200Hz giving 0x00
    // [3-0]: SLEEP_ODR=0000
#if   (ACCEL_ODR_HZ <= 1) // select 0.781Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0xC0, 0x00 },
#elif (ACCEL_ODR_HZ <= 2) // select 1.5625Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0x80, 0x00 },
#elif (ACCEL_ODR_HZ <= 3) // select 3.125Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0xA0, 0x00 },
#elif (ACCEL_ODR_HZ <= 6) // select 6.25Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0x90, 0x00 },
#elif (ACCEL_ODR_HZ <= 12) // select 12.5Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0x80, 0x00 },
#elif (ACCEL_ODR_HZ <= 25) // select 25Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0x70, 0x00 },
#elif (ACCEL_ODR_HZ <= 50) // select 50Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0x60, 0x00 },
#elif (ACCEL_ODR_HZ <= 100) // select 100Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0x50, 0x00 },
#elif (ACCEL_ODR_HZ <= 200) // select 200Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0x40, 0x00 },
#elif (ACCEL_ODR_HZ <= 400) // select 400Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0x30, 0x00 },
#elif (ACCEL_ODR_HZ <= 800) // select 800Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0x20, 0x00 },
#elif (ACCEL_ODR_HZ <= 1600) // select 1600Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0x10, 0x00 },
#else  // select 3200Hz ODR
    	{ FXLS8952_SENS_CONFIG3, 0x00, 0x00 },
#endif

    // write 0000 0011 = 0x03 to SENS_CONFIG1 to configure 4g mode and Active mode
    // [7]: RST=0 for no software reset
    // [6-5]: ST_AXIS_SEL=00 to disable self test
    // [4]: ST_POL=0 (default)
    // [3]: AWS_EN=0 (default)
    // [2-1]: FSR=01 for 4g mode
    // [1]: ACTIVE=1 for standby
    { FXLS8952_SENS_CONFIG1, 0x03, 0x00 },
    __END_WRITE_DATA__
};

// All sensor drivers and initialization functions have the same prototype.
// gbls is a pointer to the master "global" sensor fusion structure.
// sensor = pointer to linked list element used by the sensor fusion subsystem to specify required sensors

// gbls = pointer to top level (generally global) data structure for sensor fusion
int8_t FXLS8952_Init(struct PhysicalSensor *sensor, Globals *gbls)
{
    int32_t status;
    uint8_t reg;
    status = Register_I2C_Read(sensor->bus_driver, sensor->addr, FXLS8952_WHO_AM_I, 1, &reg);
    if (status==SENSOR_ERROR_NONE) {
        if (reg==FXLS8952_WHOAMI_VALUE) {
            gbls->Accel.iWhoAmI = reg;
        } else {
            return(SENSOR_ERROR_INIT);
        }
      } else {
        return(status);
    }

    // Configure and start the FXLS8952 sensor.  This does multiple register writes
    // (see FXLS8952_Initialization definition above)
    status = Sensor_I2C_Write(sensor->bus_driver, sensor->addr, FXLS8952_Initialization );

    // Stash some needed constants in the SF data structure for this sensor
    gbls->Accel.iCountsPerFloat = FXLS8952_COUNTSPERG;
    gbls->Accel.fFloatPerCount = 1.0F / FXLS8952_COUNTSPERG;
    gbls->Accel.fFloatPerCount = 1.0F / FXLS8952_COUNTSPERG;

    sensor->isInitialized = F_USING_ACCEL;
    gbls->Accel.isEnabled = true;

    return (status);
}

int8_t FXLS8952_Read(struct PhysicalSensor *sensor, Globals *gbls)
{
    uint8_t                     I2C_Buffer[6 * FXLS8952_ACCEL_FIFO_SIZE];    // I2C read buffer
    int8_t                      status;         // I2C transaction status
    int8_t                      j;              // scratch
    uint8_t                     sensor_fifo_count = 1;
    int16_t                     sample[3];

    if(sensor->isInitialized != F_USING_ACCEL)
    {
        return SENSOR_ERROR_INIT;
    }

    // read the XLS8952_BUF_STATUS register (mapped to STATUS) and extract number of measurements available (lower 6 bits)
    status =  Sensor_I2C_Read(sensor->bus_driver, sensor->addr, FXLS8952_BUF_STATUS_READ, I2C_Buffer );
    if (status==SENSOR_ERROR_NONE) {
        sensor_fifo_count = I2C_Buffer[0] & 0x3F;
        // return if there are no measurements in the sensor FIFO.
        // this will only occur when the FAST_LOOP_HZ equals or exceeds ACCEL_ODR_HZ
        if  (sensor_fifo_count == 0) return status;
    } else {
        return(status);
    }

    FXLS8952_DATA_READ[0].numBytes = 6 * sensor_fifo_count;
    status =  Sensor_I2C_Read(sensor->bus_driver, sensor->addr, FXLS8952_DATA_READ, I2C_Buffer );

    if (status==SENSOR_ERROR_NONE) {
        // process the pthisAccel->iFIFOCount * three channels of data
        for (j = 0; j < sensor_fifo_count; j++)
        {
            // place the 6 bytes read into the 16 bit accelerometer structure
            sample[CHX] = (I2C_Buffer[6 * j + 1] << 8) | I2C_Buffer[6 * j + 0];
            sample[CHY] = (I2C_Buffer[6 * j + 3] << 8) | I2C_Buffer[6 * j + 2];
            sample[CHZ] = (I2C_Buffer[6 * j + 5] << 8) | I2C_Buffer[6 * j + 4];
            conditionSample(sample);  // truncate negative values to -32767
            addToFifo((union FifoSensor*) &(gbls->Accel), ACCEL_FIFO_SIZE, sample);
        }
    }

    return (status);
}


// Each entry in a RegisterWriteList is composed of: register address, value to write, bit-mask to apply to write (0 enables)
const registerwritelist_t   FXLS8952_IDLE[] =
{
  // Set ACTIVE = other bits unchanged
  { FXLS8952_SENS_CONFIG1, 0x00, 0x01 },
    __END_WRITE_DATA__
};

// FXLS8952_Idle places the sensor into Standby mode (see datasheet for wakeup time)
int8_t FXLS8952_Idle(struct PhysicalSensor *sensor, Globals *gbls)
{
    int32_t     status;
    if(sensor->isInitialized == F_USING_ACCEL) {
        status = Sensor_I2C_Write(sensor->bus_driver, sensor->addr, FXLS8952_IDLE );
        sensor->isInitialized = 0;
        gbls->Accel.isEnabled = false;
    } else {
      return SENSOR_ERROR_INIT;
    }
    return status;
}
#endif // if F_USING_ACCEL
