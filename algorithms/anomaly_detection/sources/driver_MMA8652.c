...
int8_t MMA8652_Init(struct PhysicalSensor *sensor, Globals *gbls)
{
    int32_t status;
    uint8_t reg;
    status = Register_I2C_Read(sensor->bus_driver, sensor->addr, MMA865x_WHO_AM_I, 1, &reg);
    ...
    status = Sensor_I2C_Write(sensor->bus_driver, sensor->addr, MMA8652_Initialization );
    // Stash some needed constants in the SF data structure for this sensor
    gbls->Accel.iCountsPerFloat = MMA8652_COUNTSPERG;
    gbls->Accel.fFloatPerCount = 1.0F / MMA8652_COUNTSPERG;
    gbls->Accel.fFloatPerCount = 1.0F / MMA8652_COUNTSPERG;
...
int8_t MMA8652_Read(struct PhysicalSensor *sensor, Globals *gbls)
{
    ...
    status =  Sensor_I2C_Read(sensor->bus_driver, sensor->addr, MMA8652_F_STATUS_READ, I2C_Buffer );
    if (status==SENSOR_ERROR_NONE) {
    ...
    status =  Sensor_I2C_Read(sensor->bus_driver, sensor->addr, MMA8652_DATA_READ, I2C_Buffer );
...
int8_t MMA8652_Idle(struct PhysicalSensor *sensor, Globals *gbls)
{
    int32_t     status;
    if(sensor->isInitialized == F_USING_ACCEL) {
        status = Sensor_I2C_Write(sensor->bus_driver, sensor->addr, MMA8652_IDLE );
