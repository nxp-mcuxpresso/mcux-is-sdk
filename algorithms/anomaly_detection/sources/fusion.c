...
void fInitializeFusion(Globals *gbls)
{
    ...
    // gbls->pControlSubsystem->QuaternionPacketType = gbls->pControlSubsystem->DefaultQuaternionPacketType;
...
#if F_1DOF_P_BASIC
    gbls->SV_1DOF_P_BASIC.resetflag    = true;
#endif
#if F_3DOF_B_BASIC
    gbls->SV_3DOF_B_BASIC.resetflag    = true;
#endif
#if F_3DOF_G_BASIC
    gbls->SV_3DOF_G_BASIC.resetflag    = true;
#endif
#if F_3DOF_Y_BASIC
    gbls->SV_3DOF_Y_BASIC.resetflag    = true;
#endif
#if F_6DOF_GB_BASIC
    gbls->SV_6DOF_GB_BASIC.resetflag   = true;
#endif
#if F_6DOF_GY_KALMAN
    gbls->SV_6DOF_GY_KALMAN.resetflag  = true;
#endif
#if F_9DOF_GBY_KALMAN
    gbls->SV_9DOF_GBY_KALMAN.resetflag = true;
#endif
                          ...
                          struct GyroSensor *pthisGyro)
{
    ...
    else
    {
        ...
        for (i = CHX; i <= CHZ; i++)
        {
            if ((pthisGyro->fSum[i] >= FMIN_6DOF_GY_BPL) &&
                (pthisGyro->fSum[i] <= FMAX_6DOF_GY_BPL))
                pthisSV->fbPl[i] = pthisGyro->fSum[i];
            else
                pthisSV->fbPl[i] = 0.0F;
        }
#ifndef SIMULATION
    ...
    struct GyroSensor *pthisGyro, struct MagCalibration *pthisMagCal)
{
    ...
    // copy the gyro calibration from flash into the state vector
        ...
        // set the gyro offset to the current measurement if within limits
        for (i = CHX; i <= CHZ; i++) {
            if ((pthisGyro->fSum[i] >= FMIN_9DOF_GBY_BPL) && (pthisGyro->fSum[i] <= FMAX_9DOF_GBY_BPL))
                pthisSV->fbPl[i] = pthisGyro->fSum[i];
            else
                pthisSV->fbPl[i] = 0.0F;
        }
                       ...
                       struct GyroSensor *pthisGyro)
{
    ...
    for (i = CHX; i <= CHZ; i++) pthisSV->fOmega[i] = pthisGyro->fSum[i];
                         ...
                         struct GyroSensor *pthisGyro)
{
    ...
    if (pthisGyro->iFIFOCount > 0)
    {
        ...
        for (j = 0; j < pthisGyro->iFIFOCount; j++)
        {
            ...
            for (i = CHX; i <= CHZ; i++)
                ftmpMi3x1[i] = (float) pthisGyro->fFIFO[j][i] *
                    pthisGyro->fFloatPerCount -
                    pthisSV->fbPl[i];
                          ...
                          struct MagCalibration *pthisMagCal)
{
    ...
    // compute the average angular velocity (used for display only) from the average measurement minus gyro offset
    for (i = CHX; i <= CHZ; i++) pthisSV->fOmega[i] = (float)pthisGyro->iYs[i] * pthisGyro->fFloatPerCount - pthisSV->fbPl[i];
    ...
    if (pthisGyro->iFIFOCount > 0) {
        ...
        for (j = 0; j < pthisGyro->iFIFOCount; j++) {
        // calculate the instantaneous angular velocity subtracting the gyro offset
            for (i = CHX; i <= CHZ; i++) ftmpA3x1[i] = (float)pthisGyro->fFIFO[j][i] * pthisGyro->fFloatPerCount - pthisSV->fbPl[i];
            // compute the incremental rotation quaternion ftmpq and integrate the a priori orientation quaternion fqMi
