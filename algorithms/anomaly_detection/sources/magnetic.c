...
// function maps the uncalibrated magnetometer data fSum (uT) onto calibrated averaged data fBc (uT), iBc (counts)
void fInvertMagCal(struct MagSensor *pthisMag, struct MagCalibration *pthisMagCal)
{
    ...
    // remove the computed hard iron offsets (uT): ftmp[]=fSum[]-V[]
    for (i = CHX; i <= CHZ; i++)
    {
        ftmp[i] = pthisMag->fSum[i] - pthisMagCal->fV[i];
    }

    // remove the computed soft iron offsets (uT and counts): fBc=inv(W)*(fSum[]-V[])
    for (i = CHX; i <= CHZ; i++)
    {
                                 ...
                                 struct MagBuffer *pthisMagBuffer, struct MagSensor *pthisMag)
{
    ...
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX + 2))
    {
        ...
        ftmp = 0.5F * pthisMag->fFloatPerCount;
        for (i = CHX; i <= CHZ; i++)
        ...
        ftmp = pthisMag->fFloatPerCount / (float) pthisMagBuffer->iMagBufferCount;
        for (i = CHX; i <= CHZ; i++)
...
#define MATRIX_7_SIZE   7
    ...
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX * MAGBUFFSIZEY + 24))
    {
        ...
        pthisMagCal->ftrB = sqrtf(fabsf(ftmp)) * pthisMag->fFloatPerCount;
...
#define MATRIX_10_SIZE  10
    ...
    else if (pthisMagCal->itimeslice == (MAGBUFFSIZEX * MAGBUFFSIZEY + 48))
    {
        ...
        pthisMagCal->ftrB *= pthisMag->fFloatPerCount;
...
{
    // local variables
    float   fBs2;       // fSum[CHX]^2+fSum[CHY]^2+fSum[CHZ]^2
    ...
    fscaling = pthisMag->fFloatPerCount / DEFAULTB;
    ...
    for (j = 0; j < MAGBUFFSIZEX; j++)
    {
        for (k = 0; k < MAGBUFFSIZEY; k++)
        {
            if (pthisMagBuffer->index[j][k] != -1)
            {
                ...
                // store scaled and offset fSum[XYZ] in fvecA[0-2] and fSum[XYZ]^2 in fvecA[3-5]
                for (l = CHX; l <= CHZ; l++)
                {
                ...
                // calculate fBs2 = fSum[CHX]^2 + fSum[CHY]^2 + fSum[CHZ]^2 (scaled uT^2)
                fBs2 = pthisMagCal->fvecA[3] +
                    pthisMagCal->fvecA[4] +
                    pthisMagCal->fvecA[5];

                // accumulate fSum^4 over all measurements into fSumBs4=Y^T.Y
                fSumBs4 += fBs2 * fBs2;

                // now we have fBs2, accumulate fvecB[0-2] = X^T.Y =sum(fBs2.fSum[XYZ])
                for (l = CHX; l <= CHZ; l++)
                {
