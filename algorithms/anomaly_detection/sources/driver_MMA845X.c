...
// gbls is a pointer to the master "global" sensor fusion structure.
// sensor = pointer to linked list element used by the sensor fusion subsystem to specify required sensors

// gbls = pointer to top level (generally global) data structure for sensor fusion
int8_t MMA845x_Init(struct PhysicalSensor *sensor, Globals *gbls)
{
    int32_t status;
    uint8_t reg;
    status = Register_I2C_Read(sensor->bus_driver, sensor->addr, MMA845x_WHO_AM_I, 1, &reg);
    if (status==SENSOR_ERROR_NONE) {
        gbls->Accel.iWhoAmI = reg;
        switch (reg) {
    ...
    if (reg==MMA8451_WHO_AM_I_WHOAMI_VALUE) {
        status = Sensor_I2C_Write(sensor->bus_driver, sensor->addr, MMA8451_Initialization );
    } else {
        status = Sensor_I2C_Write(sensor->bus_driver, sensor->addr, MMA845x_Initialization );
    }

    // Stash some needed constants in the SF data structure for this sensor
    gbls->Accel.iCountsPerFloat = MMA845x_COUNTSPERG;
    gbls->Accel.fFloatPerCount = 1.0F / MMA845x_COUNTSPERG;
    gbls->Accel.fFloatPerCount = 1.0F / MMA845x_COUNTSPERG;

    sensor->isInitialized = F_USING_ACCEL;
    gbls->Accel.isEnabled = true;

    return (status);
}

int8_t MMA845x_Read(struct PhysicalSensor *sensor, Globals *gbls)
{
    ...
    status =  Sensor_I2C_Read(sensor->bus_driver, sensor->addr, MMA845x_F_STATUS_READ, I2C_Buffer );
    if (status==SENSOR_ERROR_NONE) {
        if (gbls->Accel.iWhoAmI==MMA8451_WHO_AM_I_WHOAMI_VALUE) {
    ...
    status =  Sensor_I2C_Read(sensor->bus_driver, sensor->addr, MMA845x_DATA_READ, I2C_Buffer );
    ...
    if (status==SENSOR_ERROR_NONE) {
        for (j = 0; j < sensor_fifo_count; j++)
        {
            ...
            addToFifo((union FifoSensor*) &(gbls->Accel), ACCEL_FIFO_SIZE, sample);
        }
    }
...
int8_t MMA845x_Idle(struct PhysicalSensor *sensor, Globals *gbls)
{
    int32_t     status;
    if(sensor->isInitialized == F_USING_ACCEL) {
        status = Sensor_I2C_Write(sensor->bus_driver, sensor->addr, MMA845x_IDLE );
        sensor->isInitialized = 0;
        gbls->Accel.isEnabled = false;
    } else {
      return SENSOR_ERROR_INIT;
    }
    return status;
