...
#include "anomaly_detection.h"          // Sensor fusion structures and types
...
int8_t FXOS8700_Init(struct PhysicalSensor *sensor, Globals *gbls)
{
    ...
    if (status==SENSOR_ERROR_NONE) {
       ...
       gbls->Accel.fFloatPerCount = 1.0F / FXOS8700_COUNTSPERG;
#endif
#if F_USING_MAG
       gbls->Mag.iWhoAmI = reg;
       gbls->Mag.fFloatPerCount = 1.0F / FXOS8700_COUNTSPERUT;
#endif
...
#if F_USING_ACCEL
int8_t FXOS8700_ReadAccData(struct PhysicalSensor *sensor, Globals *gbls)
{
...
int8_t FXOS8700_ReadMagData(struct PhysicalSensor *sensor, Globals *gbls)
{
    ...
    if (status==SENSOR_ERROR_NONE) {
        ...
        addToFifo((union FifoSensor*) &(gbls->Mag), MAG_FIFO_SIZE, sample);
    }
    return status;
}
#endif
...
int8_t FXOS8700_Read(struct PhysicalSensor *sensor, Globals *gbls)
{
    ...
    int8_t  sts2 = 0;
#if F_USING_ACCEL
        sts1 = FXOS8700_ReadAccData(sensor, gbls);
#endif

#if F_USING_MAG
        sts2 = FXOS8700_ReadMagData(sensor, gbls);
#endif
