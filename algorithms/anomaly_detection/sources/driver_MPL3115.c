...
int8_t MPL3115_Init(struct PhysicalSensor *sensor, Globals *gbls)
{
    int32_t status;
    uint8_t reg;
    status = Register_I2C_Read(sensor->bus_driver, sensor->addr, MPL3115_WHO_AM_I, 1, &reg);
    if (status==SENSOR_ERROR_NONE) {
        gbls->Pressure.iWhoAmI = reg;
    ...
    status = Sensor_I2C_Write(sensor->bus_driver, sensor->addr, MPL3115_Initialization );

    // Stash some needed constants in the SF data structure for this sensor
    gbls->Pressure.fmPerCount = MPL3115_MPERCOUNT; // Convert 32-bit Altitude value to meters
    gbls->Pressure.fCPerCount = MPL3115_CPERCOUNT; // Convert 16-bit Temperature value to Celcius

    sensor->isInitialized = F_USING_PRESSURE | F_USING_TEMPERATURE;
    gbls->Pressure.isEnabled = true;
...
int8_t MPL3115_Read(struct PhysicalSensor *sensor, Globals *gbls)
{
    ...
    status =  Sensor_I2C_Read(sensor->bus_driver, sensor->addr, MPL3115_DATA_READ, I2C_Buffer );

    if (status==SENSOR_ERROR_NONE) {
      	// place the read buffer into the 32 bit altitude and 16 bit temperature
	gbls->Pressure.iH = (I2C_Buffer[0] << 24) | (I2C_Buffer[1] << 16) | (I2C_Buffer[2] << 8);
	gbls->Pressure.iT = (I2C_Buffer[3] << 8) | I2C_Buffer[4];

	// convert from counts to metres altitude and C
	gbls->Pressure.fH = (float) gbls->Pressure.iH * gbls->Pressure.fmPerCount;
	gbls->Pressure.fT = (float) gbls->Pressure.iT * gbls->Pressure.fCPerCount;
    } else {
	gbls->Pressure.fH = 0.0;
	gbls->Pressure.fT = 0.0;
    }
...
int8_t MPL3115_Idle(struct PhysicalSensor *sensor, Globals *gbls)
{
    int32_t     status;
    if(sensor->isInitialized == (F_USING_PRESSURE | F_USING_TEMPERATURE)) {
        status = Sensor_I2C_Write(sensor->bus_driver, sensor->addr, MPL3115_IDLE );
        sensor->isInitialized = 0;
        gbls->Pressure.isEnabled = false;
    } else {
      return SENSOR_ERROR_INIT;
    }
    return status;
