...
/*! @brief FXLS896x Interrupt Status Register. */
const registerreadlist_t cFxls896xIntEn[] = {{.readFrom = FXLS896x_INT_STATUS, .numBytes = 1},
                                                    __END_READ_DATA__};
...
int main(void)
{
    ...
    {        /* In ISR Mode we do not need to check Data Ready Register.
        ...
        status = FXLS896x_I2C_ReadData(&fxls896xDriver, cFxls896xIntEn, &int_en);
        if (ARM_DRIVER_OK != status)
        {
            PRINTF("\r\n Read Failed. \r\n");
            return -1;
        ...
        if ((int_en & FXLS896x_INT_STATUS_SRC_DRDY_MASK) != 0x80)
        {
          continue;
        }
