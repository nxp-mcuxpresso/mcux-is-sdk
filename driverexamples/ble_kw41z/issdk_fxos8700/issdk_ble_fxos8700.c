...
/**
 * @file issdk_ble_fxos8700.c
 * @brief The issdk_ble_fxos8700.c file implements the ISSDK FXOS8700
...
gpioInputPinConfig_t gFxos8700IntCfg;
...
static int32_t FXOS8700_RegisterInterface(uint8_t opCode, uint8_t mode, uint8_t offset, uint8_t bytes, uint8_t *buffer)
{
...
void ISSDK_Initialize(void)
{
    ...
    BleApp_WriteToUART("Initializing Sensors for ISSDK FXOS8700 BLE example.\r\n");
    ...
    status = FXOS8700_I2C_Initialize(&gFxos8700Driver, &I2C_BB_DRIVER, I2C_BB_DEVICE_INDEX, FXOS8700_BB_I2C_ADDR,
    ...
    gFxos8700IntCfg.gpioPin = FXOS8700_BB_INT1.pinNumber;
    gFxos8700IntCfg.gpioPort = (gpioPort_t)FXOS8700_BB_INT1.portNumber;
        ...
        if (target != ISSDK_REPORT_NONE)
        {
            ...
            if (mode != ISSDK_REGISTER_NONE)
            {
                ...
                if (mode == ISSDK_REGISTER_READ)
                {
                    ...
                    status = FXOS8700_RegisterInterface(mode, target, offset, bytes, pBuffer);
