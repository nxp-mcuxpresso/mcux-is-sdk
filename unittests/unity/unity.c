...
static int UnityCheckArraysForNull(UNITY_PTR_ATTRIBUTE const void *expected,
...
void UnityAssertBits(
    const _U_SINT mask, const _U_SINT expected, const _U_SINT actual, const char *msg, const UNITY_LINE_TYPE lineNumber)
{
                              ...
                              const UNITY_DISPLAY_STYLE_T style)
{
    ...
    if (UnityCheckArraysForNull((UNITY_PTR_ATTRIBUTE const void *)expected, (UNITY_PTR_ATTRIBUTE const void *)actual,
                                lineNumber, msg) == 1)
        return;
                             ...
                             const UNITY_FLOAT_TRAIT_T style)
{
    ...
    switch (style)
    {
        ...
        // We are using a variable to hold the zero value because some compilers complain about dividing by zero
        // otherwise
        case UNITY_FLOAT_IS_INF:
        case UNITY_FLOAT_IS_NOT_INF:
            is_trait = ((1.0f / actual) == f_zero) ? 1 : 0;
            break;
        ...
        // NaN is the only floating point value that does NOT equal itself. Therefore if Actual == Actual, then it is
        // NOT NaN.
        ...
        case UNITY_FLOAT_IS_NOT_DET:
            if ((actual != actual) || ((1.0f / actual) == f_zero) || ((-1.0f / actual) == f_zero))
                is_trait = 0;
            else
                is_trait = 1;
            break;
...
#endif
//-----------------------------------------------
void UnityDefaultTestRun(UnityTestFunction Func, const char *FuncName, const int caseID)
{
    Unity.CurrentTestName = FuncName;

    Unity.CurrentTestId = (UNITY_LINE_TYPE)caseID;
    Unity.CurrentTestLineNumber = (UNITY_LINE_TYPE)0;
    Unity.NumberOfTests++;
    if (TEST_PROTECT())
    {
        setUp();
