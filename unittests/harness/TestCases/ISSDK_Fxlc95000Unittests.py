...
class ISSDK_Fxlc95000Unittests(unittest.TestCase):
    ...
    def test_unittest_i2c(self):
        self.driver.dutPortFlush()
        self.output.testInit("ISSDK_fxlc95000_i2c_unittests", "ISSDK Driver Examples for FXLC95000.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage(testConfiguration.FXLC95000_TEST_I2C_FILE)
            time.sleep(1)
    ...
    def test_unittest_spi(self):
        self.driver.dutPortFlush()
        self.output.testInit("ISSDK_fxlc95000_spi_unittests", "ISSDK Driver Examples for FXLC95000.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage(testConfiguration.FXLC95000_TEST_SPI_FILE)
            time.sleep(1)
...
def suite():
    ...
    suite.addTest(unittest.makeSuite(ISSDK_Fxlc95000Unittests))
