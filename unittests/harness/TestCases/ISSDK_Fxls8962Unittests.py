...
class ISSDK_Fxls8962Unittests(unittest.TestCase):
    ...
    def test_unittest_i2c(self):
        ...
        self.output.testInit("ISSDK_fxls8962_i2c_unittests", "ISSDK Driver Examples for FXLS8962.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage(testConfiguration.FXLS8962_TEST_I2C_FILE)
            time.sleep(1)
...
def suite():
    ...
    suite.addTest(unittest.makeSuite(ISSDK_Fxls8962Unittests))
