...
The Simulator will load FXAS21002 profile and will subsequently support execution of Unit-Tests.
...
FXAS21002_SAMPLE_WIDTH   = 6 # 2+2+2 Byte Gyro Data.
FXAS21002_SLAVE_ADDERSS  = '20'
BOARD_SHIELD_APPENDED    = False

class ISSDK_Fxas21002SimTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.output.testInit("ISSDK_Fxas21002SimTests", "ISSDK Unit-Tests for FXAS21002.")
    ...
    def test_unittests(self):
        try:
            ...
            (sampleCount, sampleList) = simProtocolAdapter.encodeAddSampleCommands(FXAS21002_SAMPLE_WIDTH, testConfiguration.FXAS21002_SAMPLE_FILE)
            self.output.assert_(sampleList, "Parsing Samples File Failed.", self.output.lineno())

            # Configure the Simulator for FXAS21002
            cmdPacket = simProtocolAdapter.encodeConfigureCommand(sampleCount, FXAS21002_SLAVE_ADDERSS)
            ...
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage(testConfiguration.FXAS21002_TEST_FILE)
...
def suite():
    ...
    suite.addTest(unittest.makeSuite(ISSDK_Fxas21002SimTests))
