...
MMA8491Q_RUN_TIME     = 1   # Time for example to process configured samples.
MMA8491Q_RUN_REPEATS  = 14  # Repeat reads to clean up input buffer.
BOARD_SHIELD_APPENDED = False

class ISSDK_Mma8491qExamples(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not BOARD_SHIELD_APPENDED:
            ...
            self.__class__.__name__ = "%s_A8491_%s" % (testConfiguration.BOARD_NAME, self.__class__.__name__)
        try:
    ...
    def test_example(self):
        ...
        self.output.testInit("ISSDK_mma8491q_example", "ISSDK Driver Examples for MMA8491Q.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage(testConfiguration.MMA8491Q_EG_FILE)
            fullOutput = ''
            for _ in range(MMA8491Q_RUN_REPEATS): # This example dumps a lot of output, need to keep reading at intervals.
                time.sleep(MMA8491Q_RUN_TIME) # Wait for samples to be processed.
...
def suite():
    ...
    suite.addTest(unittest.makeSuite(ISSDK_Mma8491qExamples))
