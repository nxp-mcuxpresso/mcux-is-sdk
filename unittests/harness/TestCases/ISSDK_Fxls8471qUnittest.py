...
class ISSDK_Fxls8471qUnittest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not BOARD_SHIELD_APPENDED:
            ...
            self.__class__.__name__ = "%s_%s_%s" % (testConfiguration.BOARD_NAME, FXLS8471Q_SHIELD_NAME, self.__class__.__name__)
        try:
    ...
    def test_unittest(self):
        ...
        self.output.testInit("ISSDK_fxls8471q_unittests", "ISSDK Driver Examples for FXLS8471Q.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage((testConfiguration.FXLS8471Q_TEST_FILE).replace("A8471", FXLS8471Q_SHIELD_NAME))
            time.sleep(1)
...
def suite(shield_name = "A8471"):
    ...
    suite.addTest(unittest.makeSuite(ISSDK_Fxls8471qUnittest))
