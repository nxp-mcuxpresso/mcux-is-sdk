...
class ISSDK_DP300xUnittest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not BOARD_SHIELD_APPENDED:
            ...
            self.__class__.__name__ = "%s_DP300x_%s" % (testConfiguration.BOARD_NAME, self.__class__.__name__)
        try:
    ...
    def test_unittest_i2c(self):
        ...
        self.output.testInit("ISSDK_dp300x_i2c_unittests", "ISSDK Driver Examples for DP300x.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage(testConfiguration.DP300x_TEST_FILE)
            time.sleep(1)
...
def suite():
    ...
    suite.addTest(unittest.makeSuite(ISSDK_DP300xUnittest))
