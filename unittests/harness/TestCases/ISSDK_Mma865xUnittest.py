...
class ISSDK_Mma865xUnittest(unittest.TestCase):
    ...
    def test_unittest(self):
        ...
        self.output.testInit("ISSDK_mma865x_unittests", "ISSDK Driver Examples for MMA865x.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage((testConfiguration.MMA865x_TEST_FILE).replace("MULT2B", MMA865x_SHIELD_NAME))
            time.sleep(1)
...
def suite(shield_name = "MULT2B"):
    ...
    suite.addTest(unittest.makeSuite(ISSDK_Mma865xUnittest))
