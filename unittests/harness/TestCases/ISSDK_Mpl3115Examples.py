...
class ISSDK_Mpl3115Examples(unittest.TestCase):
    ...
    def test_fifo(self):
        ...
        self.output.testInit("ISSDK_mpl3115_fifo", "ISSDK Driver Examples for MPL3115.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage((testConfiguration.MPL3115_FIFO_FILE).replace("P3115", MPL3115_SHIELD_NAME))
    ...
    def test_altitude(self):
        self.driver.dutPortFlush()
        self.output.testInit("ISSDK_mpl3115_altitude", "ISSDK Driver Examples for MPL3115.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage((testConfiguration.MPL3115_ALT_FILE).replace("P3115", MPL3115_SHIELD_NAME))
            time.sleep(MPL3115_START_TIME)
            fullOutput = ''
            for _ in range(MPL3115_RUN_REPEATS): # This example dumps a lot of output, need to keep reading at intervals.
                time.sleep(MPL3115_RUN_TIME) # Wait for samples to be processed.
    ...
    def test_normal(self):
        self.driver.dutPortFlush()
        self.output.testInit("ISSDK_mpl3115_normal", "ISSDK Driver Examples for MPL3115.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage((testConfiguration.MPL3115_NORM_FILE).replace("P3115", MPL3115_SHIELD_NAME))
            time.sleep(MPL3115_START_TIME)
            fullOutput = ''
            for _ in range(MPL3115_RUN_REPEATS): # This example dumps a lot of output, need to keep reading at intervals.
                time.sleep(MPL3115_RUN_TIME) # Wait for samples to be processed.
    ...
    def test_1shot(self):
        ...
        time.sleep(1)
        self.driver.dutPortFlush()
        self.output.testInit("ISSDK_mpl3115_oneshot", "ISSDK Driver Examples for MPL3115.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage((testConfiguration.MPL3115_ONE_FILE).replace("P3115", MPL3115_SHIELD_NAME))
            time.sleep(MPL3115_START_TIME)
...
def suite(shield_name = "P3115"):
    ...
    suite.addTest(unittest.makeSuite(ISSDK_Mpl3115Examples))
