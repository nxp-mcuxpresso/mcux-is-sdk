...
The Simulator will load FXOS8700 profile and will subsequently support execution of Unit-Tests.
...
FXOS8700_SAMPLE_WIDTH   = 6  # 2+2+2 Byte of XYZ Accel/Mag
FXOS8700_SLAVE_ADDERSS  = '1E'
BOARD_SHIELD_APPENDED   = False

class ISSDK_Fxos8700SimTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.output.testInit("ISSDK_Fxos8700SimTests", "ISSDK Unit-Tests for FXOS8700.")
    ...
    def test_unittests(self):
        try:
            ...
            (sampleCount, sampleList) = simProtocolAdapter.encodeAddSampleCommands(FXOS8700_SAMPLE_WIDTH, testConfiguration.FXOS8700_SAMPLE_FILE)
            self.output.assert_(sampleList, "Parsing Samples File Failed.", self.output.lineno())

            # Configure the Simulator for FXOS8700
            cmdPacket = simProtocolAdapter.encodeConfigureCommand(sampleCount, FXOS8700_SLAVE_ADDERSS)
            ...
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage(testConfiguration.FXOS8700_TEST_FILE)
...
def suite():
    ...
    suite.addTest(unittest.makeSuite(ISSDK_Fxos8700SimTests))
