...
class ISSDK_Fxas21002Examples(unittest.TestCase):
    ...
    def test1_poll(self):
        ...
        self.output.testInit("ISSDK_fxas21002_poll", "ISSDK Driver Examples for FXAS21002.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage((testConfiguration.FXAS21002_POLL_FILE).replace("AGM01", FXAS21002_SHIELD_NAME))
    ...
    def test3_fifo(self):
        self.driver.dutPortFlush()
        self.output.testInit("ISSDK_fxas21002_fifo", "ISSDK Driver Examples for FXAS21002.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage((testConfiguration.FXAS21002_FIFO_FILE).replace("AGM01", FXAS21002_SHIELD_NAME))
            time.sleep(FXAS21002_RUN_TIME) # Wait for samples to be processed.
...
def suite(shield_name = "AGM01"):
    ...
    suite.addTest(unittest.makeSuite(ISSDK_Fxas21002Examples))
