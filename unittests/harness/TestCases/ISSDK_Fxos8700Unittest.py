...
class ISSDK_Fxos8700Unittest(unittest.TestCase):
    ...
    def test_unittest(self):
        ...
        self.output.testInit("ISSDK_fxos8700_unittests", "ISSDK Driver Examples for FXOS8700.")
        print("\n\n****************** Executing Test  ******************\n")
        try:
            # Load the DUT image onto the DUT Board.
            testConfiguration.BOARD_OBJECT_HANDLE.loadDutImage((testConfiguration.FXOS8700_TEST_FILE).replace("AGM01", FXOS8700_SHIELD_NAME))
            time.sleep(1)
...
def suite(shield_name = "AGM01"):
    ...
    suite.addTest(unittest.makeSuite(ISSDK_Fxos8700Unittest))
