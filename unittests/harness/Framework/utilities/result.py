...
# Modified file required to support bamboo readable XML files.
# The original file in python installation library should be overwritten with this file.
...
class _XMLTestResult(_TextTestResult):
    ...
    def stopTest(self, test):
        ...
        # Bamboo: Since buffering is on for XML, display test output on completion.
        print self._stdout_data
    ...
    def _test_class_name(test_id):
        """
        Returns the test method name.
        """
        return test_id.split('.')[1]

    _test_class_name = staticmethod(_test_class_name)
    ...
    def _report_testcase_bamboo(suite_name, test, xml_document, parentElement,
                          properties):
        """
        Bamboo: Generate custom testcase sections for XML results file.
        """
        testcase = xml_document.createElement('testcase')
        parentElement.appendChild(testcase)

        testcase.setAttribute('classname', _XMLTestResult._test_class_name(test.test_id))
        testcase.setAttribute(
            'name', _XMLTestResult._test_method_name(test.test_id)
        )
        testcase.setAttribute('time', '%.3f' % test.elapsed_time)

        if (test.stdout.find("Result: Passed") == -1):
            elem_name = 'failure'
        else:
            elem_name = 'system-out'
        systemout = xml_document.createElement(elem_name)
    ...
    _report_testcase_bamboo = staticmethod(_report_testcase_bamboo)
    ...
    def generate_reports(self, test_runner):
        ...
        for _, tests in all_results.items():
            for test in tests:
                if outputHandledAsString:
                    doc = Document()
                    parentElement = doc

                suite_name = _XMLTestResult._test_class_name(test.test_id) + '_' + _XMLTestResult._test_method_name(test.test_id)
                if test_runner.outsuffix:
                    # not checking with 'is not None', empty means no suffix.
                    suite_name = '%s-%s' % (suite_name, test_runner.outsuffix)

                """
                # Bamboo: Disable standard XML file formating.
                # Build the XML file
                testsuite = _XMLTestResult._report_testsuite(
                    suite_name, tests, doc, parentElement, self.properties
                )

                # Bamboo: Build custom XML file for Bamboo.
                """
                testsuite = _XMLTestResult._report_testcase_bamboo(
                    suite_name, test, doc, parentElement, self.properties
                )
