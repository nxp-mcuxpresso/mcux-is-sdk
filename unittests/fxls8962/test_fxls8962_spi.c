...
fxls8962_spi_sensorhandle_t gFxls8962Driver;
...
void test_fxls8962_negative_1(void)
{
    ...
    status = FXLS8962_SPI_Initialize(&gFxls8962Driver, corruptedSPIdriver, SPI_S_DEVICE_INDEX, &FXLS8962_CS,
    ...
    status = FXLS8962_SPI_ReadData(&gFxls8962Driver, cFxls8962WhoAmICommand, &whoAmI);

    // CHECK - Check Sensor Read Status
    TEST_ASSERT_EQUAL_INT(SENSOR_ERROR_INIT, status);

    // Configure the FXLS8962 sensor driver with Normal mode.
    status = FXLS8962_SPI_Configure(&gFxls8962Driver, cFxls8962ConfigNormal);
...
void test_fxls8962_negative_2(void)
{
    ...
    // Initialize the FXLS8962 sensor driver.
    status = FXLS8962_SPI_Initialize(&gFxls8962Driver, &SPI_S_DRIVER, SPI_S_DEVICE_INDEX, badSlaveHandle,
                                     FXLS8962_WHOAMI_VALUE);
...
void test_fxls8962_init(void)
{
    ...
    status = FXLS8962_SPI_Initialize(&gFxls8962Driver, &SPI_S_DRIVER, SPI_S_DEVICE_INDEX, &FXLS8962_CS,
                                     FXLS8962_WHOAMI_VALUE);
...
void test_fxls8962_negative_3(void)
{
    ...
    status = FXLS8962_SPI_Configure(&gFxls8962Driver, pBadRegWriteList);
...
void test_fxls8962_config(void)
{
    ...
    FXLS8962_SPI_ReadData(&gFxls8962Driver, cFxls8962SENS_CONFIG3, &regBuf);
    ...
    FXLS8962_SPI_ReadData(&gFxls8962Driver, cFxls8962SENS_CONFIG1, &regBuf);
    ...
    FXLS8962_SPI_ReadData(&gFxls8962Driver, cFxls8962INT_EN, &regBuf);
...
void test_fxls8962_negative_4(void)
{
    ...
    // Reading sensor data with bad read register list
    status = FXLS8962_SPI_ReadData(&gFxls8962Driver, badRegReadList, &whoAmI);
...
void test_fxls8962_deInit(void)
{
    ...
    status = FXLS8962_SPI_Deinit(&gFxls8962Driver);
