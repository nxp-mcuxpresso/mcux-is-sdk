...
/*! @file  stream_mpl3115.c
*   @brief This is the mpl3115 sensor driver streaming test file.
*/
...
int main(void)
{
    ...
    PRINTF("\r\n[DUT] ISSDK MPL3115 sensor driver example demonstration with poll mode\r\n");

    /*! Initialize the I2C driver. */
    status = I2Cdrv->Initialize(I2C_S_SIGNAL_EVENT);
    if (ARM_DRIVER_OK != status)
    {
        PRINTF("\r\n[DUT] I2C Initialization Failed\r\n");
        return -1;
    }
    ...
    if (ARM_DRIVER_OK != status)
    {
        PRINTF("\r\n[DUT] I2C Power Mode setting Failed\r\n");
        return -1;
    }
    ...
    if (ARM_DRIVER_OK != status)
    {
        PRINTF("\r\n[DUT] I2C Control Mode setting Failed\r\n");
        return -1;
    }
    ...
    {
        PRINTF("\r\n[DUT] Sensor Initialization Failed\r\n");
        return -1;
    }
    PRINTF("\r\n[DUT] Successfully Initiliazed Sensor\r\n");
    ...
    if (SENSOR_ERROR_NONE != status)
    {
        PRINTF("\r\n[DUT] MPL3115 Configuration failed\r\n");
        return -1;
    }
    PRINTF("\r\n[DUT] Successfully Applied MPL3115 Sensor Configuration\r\n");

    for (int16_t count = 0;;) /* Forever loop */
    {
        ...
        if (count % 5000 == 0)
        {
            PRINTF("\r\n[DUT] Samples Received [%05d]\r\n", count + 1);
        }
        if (count != rawData.temperature)
        {
            PRINTF("\r\n[DUT] %d Incremental sequence Temperature Samples Received.\r\n", count);
            count = rawData.temperature;
        }
        count++;
    }
}
