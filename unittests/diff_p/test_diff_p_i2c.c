...
/*! File: test_diff_p_i2c.c
* @brief The test_diff_p_i2c.c file contains unittest for DIFF-P I2C sensor
...
diff_p_i2c_sensorhandle_t gDiff_pDriver;
...
void test_diff_p_negative_1(void)
{
    ...
    PRINTF("\r\nInitialize DIFF_P Sensor with corrupted I2C driver metadata\r\n");
    status = DIFF_P_I2C_Initialize(&gDiff_pDriver, corruptedI2Cdriver, I2C_S_DEVICE_INDEX, DIFF_P_I2C_ADDR,
    ...
    PRINTF("\r\nRead DIFF_P data.\r\n");
    status = DIFF_P_I2C_ReadData(&gDiff_pDriver, diff_p_WHO_AM_I, &whoAmI);
    ...
    status = DIFF_P_I2C_Configure(&gDiff_pDriver, cDiffPConfigNormal);
...
void test_diff_p_negative_2(void)
{
    ...
    // Initialize the DIFF_P sensor driver.
    PRINTF("\r\nInitialize DIFF_P Sensor with bad I2C slave address\r\n");
    status = DIFF_P_I2C_Initialize(&gDiff_pDriver, &I2C_S_DRIVER, I2C_S_DEVICE_INDEX, badSlaveaddr,
                                   DIFF_P_NPS3000VV_WHOAMI_VALUE);
...
void test_diff_p_init(void)
{
    ...
    status = DIFF_P_I2C_Initialize(&gDiff_pDriver, &I2C_S_DRIVER, I2C_S_DEVICE_INDEX, DIFF_P_I2C_ADDR,
                                   DIFF_P_NPS3000VV_WHOAMI_VALUE);
...
void test_diff_p_negative_3(void)
{
    ...
    status = DIFF_P_I2C_Configure(&gDiff_pDriver, badRegWriteList);
...
void test_diff_p_config(void)
{
    ...
    status = DIFF_P_I2C_ReadData(&gDiff_pDriver, diff_p_CTRL_REG1, &ctrlReg1);

    // CHECK - Check CTRL_REG1 value
    TEST_ASSERT_EQUAL_HEX(CTRLREG1_TESTVAL, ctrlReg1);

    status = DIFF_P_I2C_ReadData(&gDiff_pDriver, diff_p_CTRL_REG2, &ctrlReg2);
    ...
    status = DIFF_P_I2C_ReadData(&gDiff_pDriver, diff_p_CTRL_REG3, &ctrlReg3);
    ...
    status = DIFF_P_I2C_ReadData(&gDiff_pDriver, diff_p_INT_MASK0, &intMask0);
...
void test_diff_p_negative_4(void)
{
    ...
    status = DIFF_P_I2C_ReadData(&gDiff_pDriver, badRegReadList, &whoAmI);
...
void test_diff_p_deInit(void)
{
    ...
    status = DIFF_P_I2C_DeInit(&gDiff_pDriver);
