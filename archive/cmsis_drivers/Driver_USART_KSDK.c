...
#define _UART_INSTANCE_BASE (UART_Type *) BOARD_DEBUG_UART_BASEADDR
...
//
/* UART user callback */
void KSDK_UART0_UserCallback(UART_Type *base, uart_handle_t *handle, status_t status, void *userData)
{
    userData = userData;
    uint32_t event;
...
int32_t KSDK_UART0_USART_Initialize(ARM_USART_SignalEvent_t cb_event)
{
    ...
    UART_Init(_UART_INSTANCE_BASE, &config, CLOCK_GetFreq(_UART_CLKSRC_));
    UART_TransferCreateHandle(_UART_INSTANCE_BASE, &g_uart0Handle, KSDK_UART0_UserCallback, NULL);
    UART_TransferStartRingBuffer(_UART_INSTANCE_BASE, &g_uart0Handle, g_uart0_ringbuffer, RX_RING_BUFFER_SIZE);
    callBack = cb_event;
...
int32_t KSDK_UART0_USART_Send(const void *data, uint32_t num)
{
    ...
    xfer.dataSize = num;
    if (kStatus_Success != UART_TransferSendNonBlocking(_UART_INSTANCE_BASE, &g_uart0Handle, &xfer))
    {
...
{
    uart_transfer_t recvxfer;
    recvxfer.data = data;
    recvxfer.dataSize = num;
    if (kStatus_Success != UART_TransferReceiveNonBlocking(_UART_INSTANCE_BASE, &g_uart0Handle, &recvxfer, NULL))
    {
        return ARM_DRIVER_ERROR;
    }
