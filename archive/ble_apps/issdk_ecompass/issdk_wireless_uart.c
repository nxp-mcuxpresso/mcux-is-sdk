/*! *********************************************************************************
 * \addtogroup Wireless UART ISSDK Application
 * @{
 ********************************************************************************** */
/*!
 ...
 * ile
 *
 * This file is the source file for the ISSDK FXOS8700 driven Wireless UART.
 * All rights reserved.
 *
...
#define mBatteryLevelReportSkipCount_c  (10)	/* battery level report skip count */

#define mMeasurementReportInterval_c    (1)	   /* measurement report interval in seconds  */
/************************************************************************************
 *************************************************************************************
...
void BleApp_Init(void)
{
    ...
    /* Default Reporting Mode is Pedometer. */
    gReportMode = ISSDK_REPORT_DATA;
    BleApp_WriteToUART("\r\n Default Report Mode is Pedometer.\r\n");
}
    ...
    switch (pConnectionEvent->eventType)
    {
        case gConnEvtConnected_c:
        {
            ...
            TMR_StartLowPowerTimer(mMeasurementTimerId, gTmrLowPowerIntervalMillisTimer_c,
                       TmrSeconds(mMeasurementReportInterval_c), MeasurementTimerCallback, NULL);
            ISSDK_ConfigureSensor(ISSDK_START_SENSOR);
...
//                    Gap_Pair(peerDeviceId, &gPairingParameters);
        ...
        case gConnEvtDisconnected_c:
        {
            ...
            ISSDK_ConfigureSensor(ISSDK_STOP_SENSOR);
...
static void BleApp_ReceivedUartStream(uint8_t *pStream, uint16_t streamLength)
{
    ...
    if (0 == strncasecmp(pStream, ISSDK_REPORT_DATA_STR, strlen(ISSDK_REPORT_DATA_STR)))
    {
        gReportMode = ISSDK_REPORT_DATA;
        BleApp_WriteToUART("\r\n Switching To RAW DATA Mode.\r\n");
    }
    else if (0 == strncasecmp(pStream, ISSDK_REPORT_STEPS_STR, strlen(ISSDK_REPORT_STEPS_STR)))
    {
        gReportMode = ISSDK_REPORT_STEPS;
        BleApp_WriteToUART("\r\n Switching To PEDOMETER Mode.\r\n");
    }
...
/*! *********************************************************************************
* \brief        Handles Sensor data and battery measurement timer callback.
*
* \param[in]    pParam        Calback parameters.
********************************************************************************** */
static void MeasurementTimerCallback(void * pParam)
{
	static uint8_t batteryMeasurementSkipCounter = mBatteryLevelReportSkipCount_c;

    if(batteryMeasurementSkipCounter++ == mBatteryLevelReportSkipCount_c)
    {
        mBasServiceConfig.batteryLevel = BOARD_GetBatteryLevel();
        Bas_RecordBatteryMeasurement(mBasServiceConfig.serviceHandle, mBasServiceConfig.batteryLevel);
        batteryMeasurementSkipCounter = 0;
    }

    if(gReportMode == ISSDK_REPORT_STEPS)
    {
        ISSDK_ReadPedometer();
    }

    if(gReportMode == ISSDK_REPORT_DATA)
    {
        ISSDK_ReadSensor();
    }
}
...
void BleApp_WriteToHost(char *format, ...)
{
    ...
    if(pStr != NULL)
    {
        ...
        BleApp_SendUartStream(mPeerInformation.deviceId, pStr, n);
...
void BleApp_WriteToUART(char *format, ...)
{
    ...
    if(pStr != NULL)
    {
        va_start(ap, format);
        n = vsnprintf(pStr, mAppUartBufferSize_c, format, ap);
        Serial_AsyncWrite(gAppSerMgrIf, pStr, n, Uart_TxCallBack, NULL);
        /* Memory will be freed when TX completes */
    }
}
