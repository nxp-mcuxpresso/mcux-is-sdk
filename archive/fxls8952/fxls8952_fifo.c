...
/*! @file  fxls8952_fifo.c
*   @brief This is the fxls8952 sensor driver demo file for FIFO Mode.
*/
...
const registerreadlist_t cFxls8952FIFOEvent[] = {{.readFrom = FXLS8952_BUF_STATUS, .numBytes = 1}, __END_READ_DATA__};
...
int main(void)
{
    ...
    for (;;)
    {
        ...
        status = FXLS8952_I2C_ReadData(&fxls8952Driver, cFxls8952FIFOEvent, (uint8_t *)&gFxls8952DataReady);
        if (0 == (gFxls8952DataReady & FXLS8952_BUF_STATUS_WMRK_MASK))
        ...
        { /*! Process all samples and convert the raw sensor data. */
            ...
            ASK_USER_TO_RESUME(1000); /* Ask for user input after processing 100 samples. */
        }
