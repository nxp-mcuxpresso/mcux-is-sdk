...
#define STREAMING_PAYLOAD_LEN                                                                                       \
    (TIMESTAMP_DATA_SIZE + NINEAXISSENSOR_DATA_SIZE + QUATERNION_SIZE + EULERANGLE_DATA_SIZE + FITERROR_DATA_SIZE + \
     COORDINATES_SIZE + BOARDINFO_SIZE + BUILDNAME_SIZE + SYSTICKINFO_SIZE)
...
void encodeOrientPacketStream(SensorFusionGlobals *sfg, uint8_t *sUARTOutputBuffer)
{
    ...
    /*! Add Accel X, Y Z (6 bytes) value to Orientation Buffer. Send non-zero data only if the accelerometer sensor is
     * enabled */
    ...
    /*! Add MAG X, Y Z (6 bytes) value to Orientation Buffer. Send non-zero data only if the magnetometer sensor is
     * enabled */
...
/* Set the coordinate system bits in flags from default NED (00) */
    ...
    // if (true) //(gStreamingEnabled) /* For Testing Sending Orientation streaming packet always till we have host side
    // application
    {
