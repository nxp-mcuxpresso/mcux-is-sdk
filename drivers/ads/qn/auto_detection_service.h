 ...
 * service that automatically detects the QN board and Sensor Shield.
 */
...
typedef struct
{
    uint32_t mcuChipId;          // MCU Specific Chip ID
    uint32_t mcuChipIdMask;      // MCU Chip ID CID3 Bits to match (not all bits are defined)
    ADS_SupportedBoards_t board; // MCU Board corresponding to Chip ID value.
} mcuDEVID_t;
