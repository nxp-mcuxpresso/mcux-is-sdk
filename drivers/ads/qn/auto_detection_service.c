 ...
 *        for discovery of QN Board and Sensor Shield Board.
 */
...
/*!
* @brief This constant data structure contains the expected valid MCU Chip ID combinations.
* Refer : QN908x user manual - UM11023, Section: 2.5.10
*/
const mcuDEVID_t ADS_ValidDEVIDValues[] = {
    {0x00, 0x3F, ADS_QN_9020}, // QN9020
    {0x10, 0x38, ADS_QN_9080}, // QN908x
};
...
const sensorAccess_t ADS_SensorQueryList[] = {
    ...
    {ADS_I2C_INT, MMA8652_I2C_ADDR, MMA865x_WHO_AM_I, MMA8652_WHOAMI_VALUE, 0xFF},
};
...
int ADS_DetectQN(char *pBoardString, size_t bufferLength)
{
    ...
    mcuDEVID_t MCUInfo = {.mcuChipId = 0, .board = ADS_NO_BOARD_DETECTED};
    ...
    // Get CID3 (CHIP ID for product ID) from SYSCON CHIP_ID register.
    MCUInfo.mcuChipId = (SYSCON->CHIP_ID & SYSCON_CHIP_ID_CID3_MASK) >> SYSCON_CHIP_ID_CID3_SHIFT;
    ...
    {
        if ((ADS_ValidDEVIDValues[i].mcuChipIdMask & MCUInfo.mcuChipId) ==
            (ADS_ValidDEVIDValues[i].mcuChipIdMask & ADS_ValidDEVIDValues[i].mcuChipId))
        {
    ...
    {
        case ADS_QN_9020:
            strncpy(pBoardString, "QN9020DK", bufferLength);
            break;
        case ADS_QN_9080:
            strncpy(pBoardString, "QN908XCDK", bufferLength);
            break;
        default:
            status = ARM_DRIVER_ERROR;
            break;
...
ADS_Status_t ADS_FlashUpdate(char *pResultString)
{
    ...
    // Load default Flash configuration.
    ...
    if (kStatus_FLASH_Success == result)
    {
        ...
        result = FLASH_Erase(&s_flashDriver, ADS_NVM_ADDR, FSL_FEATURE_FLASH_PAGE_SIZE_BYTES);
        if (kStatus_FLASH_Success == result)
        {
            // Write the ADS Record to the Flash.
            result = FLASH_Program(&s_flashDriver, ADS_NVM_ADDR, (uint32_t *)&flashRecord, sizeof(flashRecord));
            if (kStatus_FLASH_Success == result)
            {
                retVal = ADS_OK;
            }
...
void BOARD_RunADS(const char *appName, char *boardString, char *shieldString, size_t bufferLength)
{
    ...
    status = ADS_DetectQN(boardString, bufferLength);
